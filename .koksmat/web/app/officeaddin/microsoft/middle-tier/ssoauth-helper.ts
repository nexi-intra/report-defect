...
export async function getAccessToken(authorization: string): Promise<any> {
  if (!authorization) {
    ...
    const accessAsUserScope = tokenScopes.find((scope: string) => scope === "access_as_user");
...
export function validateJwt(req: { headers: { authorization: any; }; }, res: { sendStatus: (arg0: number) => void; }, next: () => void): void {
...
function getSigningKeys(header: any, callback: any) {
  ...
  client.getSigningKey(header.kid, function (_err, key : any) {
