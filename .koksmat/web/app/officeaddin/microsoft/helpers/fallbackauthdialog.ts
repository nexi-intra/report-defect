...
import { AuthenticationResult, Configuration, LogLevel, PublicClientApplication, RedirectRequest } from "@azure/msal-browser";
import { callGetUserData } from "./middle-tier-calls";


const clientId = "64599827-012f-4b98-a7ed-52c92947df32"; //This is your client ID
...
let loginDialog: Office.Dialog | null = null;
let homeAccountId : any = null;
...
export async function dialogFallback(callback: any) {
...
async function processMessage(args: { 
  ...
  let messageFromDialog = JSON.parse(msg.message);
  ...
  if (messageFromDialog.status === "success") {
    ...
    if (messageFromDialog.error) {
      alert(JSON.stringify(messageFromDialog.error.toString()));
    } else if (messageFromDialog.result) {
      alert(JSON.stringify(messageFromDialog.result.errorMessage.toString()));
    }
  }
}
